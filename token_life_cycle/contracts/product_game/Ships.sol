...
contract Ships {
    ...
    mapping(uint => uint) public aimJourney;
    mapping(address => bool) public roleCall;
    mapping(uint => bool) public shipBuilded;
    mapping(uint => uint) public startJourney;
    ...
    function _finishShip(
        address _who, 
        uint _shipyardId, 
        uint _shipId
    )
        external 
        onlyOwner(_who, _shipId) 
        shipExist(_shipId) 
        onlyRole(msg.sender)
    {
    ...
    function _calculateRange(uint _id, uint _finishLandId)private view shipExist(_id) returns(uint result){
